...
function enhanceResponse(res) {
  ...
  res.sendFile = (filePath) => {
    ...
    import_fs.default.createReadStream(absPath).on("error", () => {
      res.writeHead(404);
...
async function cookies(req, res) {
  req.cookies = {};
...
function serveStatic(root) {
  return async function serveStaticMiddleware(req, res) {
    ...
    const filePath = import_path2.default.join(process.cwd(), root, urlPath);
    try {
      const stat = await import_fs2.default.promises.stat(filePath);
...
// routers/api/auth.js
var auth_default = async (req, res) => {
...
var api_default = async (req, res) => {
  ...
  if (!await auth_default(req, res)) return false;
  if (!await users_default(req, res)) return false;
  return true;
};

// routers/error.js
async function _error(req, res) {
...
// app.js
var fileServer = serveStatic("public");
import_http.default.createServer(async (req, res) => {
  try {
    ...
    if (!await fileServer(req, res)) return false;
    if (!await api_default(req, res)) return false;
    if (!await home(req, res)) return false;
    if (!await _error(req, res)) return false;
  } catch (err) {
    handleError(err, res);
  }
}).listen(3e3, () => {
